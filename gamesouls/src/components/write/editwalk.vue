	...
	<v-container>
		<v-row justify="center" width="800px" class="mt-6">
			<v-card style="width: 800px;" class="px-6" tile>
				<v-row>
				<v-text-field prepend-icon="mdi-format-title" color="yellow" v-model="title" readonly label="攻略标题"></v-text-field>
...
<script>
	export default {
		...
		created() {
			this.$axios.get('http://localhost:8888/getwalktitle/'+this.$route.params.id).then((response)=>{
	    ...
	    methods: {
			...
			upload(){
				...
				this.$axios.post('http://localhost:8888/addcheck/',data).then((response)=>{
					this.snackbar=true;
					this.$router.go(-1);
				})
