<template>
	...
	<v-container class="py-4">
		<v-toolbar color="white" width="800px" class="mx-auto my-2">
		    <v-toolbar-title>反馈管理</v-toolbar-title>
		    <v-spacer></v-spacer>
		</v-toolbar>
		<template v-for="(item, index) in items">
		<v-card width="800px" class="mx-auto my-2" tile>
			<v-card-title>{{item.username}}<v-spacer></v-spacer>
			<v-icon @click="deletefeedback(index)">mdi-delete</v-icon></v-card-title>
	...
	export default {
		...
		methods:{
			deletefeedback(index){
				this.$axios.get('api/deletefeedback',{
				        params:{
		...
		created() {
			...
			this.$axios.get('api/getfeedbacklist').then((response)=>{
			    var a = response.data;
