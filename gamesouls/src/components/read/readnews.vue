<template>
	<v-container>
		...
		<v-card width="800px" tile>
			<v-row justify="center" class="display-1 pt-12 pb-12 px-4">{{title}}</v-row>
			<v-row align="center" class="px-12 pb-6">
				...
				<v-spacer></v-spacer>
				<v-btn small fab dark color="cyan" class="mx-1" @click="addheart"><v-icon>mdi-heart-outline</v-icon></v-btn>
				<v-btn small fab dark color="cyan" class="mx-1" @click="addcollect"><v-icon>mdi-bookmark-outline</v-icon></v-btn>
				<v-btn small fab dark color="cyan" class="ml-1" @click="send"><v-icon>mdi-share-outline</v-icon></v-btn>
			</v-row>
			<v-row class="px-12">
				<v-sheet v-html="text"></v-sheet>
			</v-row>
			<v-row class="px-12 py-6">
				<v-btn rounded dark color="cyan" @click="addheart"><v-icon>mdi-heart-outline</v-icon>{{heart}}</v-btn>
				<v-btn rounded dark color="cyan" class="ml-1 mr-4" @click="addcollect"><v-icon>mdi-bookmark-outline</v-icon>{{collect}}</v-btn>
				<v-spacer></v-spacer>
				<v-btn small fab dark color="cyan" class="mx-1" @click="addreport"><v-icon>mdi-comment-alert-outline</v-icon></v-btn>
				<v-btn small fab dark color="cyan" class="mx-1" @click="send"><v-icon>mdi-share-outline</v-icon></v-btn>
				<v-btn small fab dark color="cyan" class="ml-1" @click="deletenews" v-show="showdelete">
		...
		<v-card width="800px" tile>
			...
			<v-divider></v-divider>
			<v-row>
			<v-textarea color="cyan" class="pl-6 pt-6" label="评论" v-model="commenttext" outlined clearable></v-textarea>
			<v-btn text color="cyan" class="mt-6 mr-6 ml-2" tile @click="addcomment">提交</v-btn></v-row>
			<v-divider></v-divider>
			<v-list>
	...
	export default{
		data(){
			return{
				...
				username:'',commenttext:'',avatar:'',cover:'',id:'',
				title:'',
				date:'',
		...
		created(){
			this.$axios.get('http://localhost:8888/getnews/'+this.$route.params.id).then((res)=>{
			...
			this.$axios.get('http://localhost:8888/getnewscommentlist/'+this.$route.params.id).then((res)=>{
		...
		methods:{
			deletenews(){
				this.$axios.get('http://localhost:8888/deletenews/'+this.id).then((response)=>{
			...
			addheart(){
				...
				this.$axios.get('http://localhost:8888/heartnews/',{
				        params:{
				           id:this.id,
						   userid:this.userid,
			...
			addcollect(){
				...
				this.$axios.get('http://localhost:8888/addnewscollect/',{
				        params:{
				           id:this.id,
			...
			addreport(){
				...
				this.$axios.get('http://localhost:8888/reportnews/',{
				        params:{
						   userid:localStorage.getItem("id"),
						   username:localStorage.getItem("username"),
						   reportid:this.id,
			...
			addcomment(){
				...
				this.$axios.get('http://localhost:8888/addnewscomment/',{
				        params:{
						   userid:localStorage.getItem("id"),
						   username:localStorage.getItem("username"),
